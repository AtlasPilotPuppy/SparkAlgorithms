...
object knnJoin_Int {
   ...
   *
   * @param rdd : RDD of Vectors of Int, which is the data-set in which knnJoin has to be 
   * 		undertaken
  ...
  def knnJoin_perIteration(rdd : RDD[(Vector[Int],Long)],
   ...
   /*
   if the no.of entries in the greaterRDD less than <len>  extract all entries from
   greaterRDD and <len> + (<len> - greaterRDD.count) no.of entries from lesserRDD
   */
  ...
  def knnJoin(dataSet : RDD[Vector[Int]],
   ...
   for(count <- 2 to randomSize) {
     ...
     //remove the effect of random vector "rand" from each entry of the the returned RDD 
     //from knnJoin_perIteration
     var z_Looped = -1
  ...
  /**
   * Performs kNN over the modified data-set and returns the k-nearest neighbors for 
   * the data-point
   *
   * @param reducedData : RDD of Vector of Int, which is the reduced data-set after kNNJoin
   ...
   */
  def zKNN(reducedData : RDD[Vector[Int]],
      dataPoint : Vector[Int], k : Int) : RDD[Vector[Int]] = {
